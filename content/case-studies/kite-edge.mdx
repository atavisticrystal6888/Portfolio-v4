...
<figure class="case-artifact">
  <img src="/images/projects/kite-edge/quantstats-tearsheet.png" alt="QuantStats tear sheet generated by KiteEdge's report module on synthetic demo returns: cumulative-return charts against a benchmark and the key performance metrics table — Sharpe, Sortino, max drawdown, volatility, VaR." loading="lazy" />
  <figcaption>The platform's own tear-sheet module (QuantStats 0.0.81) run on synthetic demo returns — seeded RNG, 248 trading days, benchmark alongside. It's the exact code path that renders a user's real holdings when self-hosted; the title says "Synthetic Demo Data" because real account data never leaves the machine, mine included.</figcaption>
</figure>
