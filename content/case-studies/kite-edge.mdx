...
- **Analytics only.** The platform must never execute a trade — zero regulatory exposure, and the user never has to trust it with an order. This boundary was set before the first endpoint and shaped every integration.
- **Self-hosted privacy.** Kite OAuth tokens live only in Redis with an 18-hour TTL, never on disk; no data leaves the user's machine. Trust is the differentiator, so it's enforced by architecture, not policy.
- **Free data sources.** No Bloomberg budget — fundamentals coverage is best-effort and nullable by design, and every derived number has to carry its methodology instead of implying terminal-grade completeness.
...
<figure class="case-artifact">
  <a href="/images/projects/kite-edge/quantstats-tearsheet.png" target="_blank" rel="noopener noreferrer"><img src="/images/projects/kite-edge/quantstats-tearsheet.png" alt="QuantStats tear sheet generated by KiteEdge's report module on synthetic demo returns: cumulative-return charts against a benchmark and the key performance metrics table — Sharpe, Sortino, max drawdown, volatility, VaR." loading="eager" /></a>
  <figcaption>The platform's own tear-sheet module (QuantStats 0.0.81) run on synthetic demo returns — seeded RNG, 248 trading days, benchmark alongside. It's the exact code path that renders a user's real holdings when self-hosted; the title says "Synthetic Demo Data" because real account data never leaves the machine, mine included. Click the sheet to open it full size.</figcaption>
</figure>

<!-- v5 WS-C: KiteEdge product photos still to come (user-supplied, last). -->
