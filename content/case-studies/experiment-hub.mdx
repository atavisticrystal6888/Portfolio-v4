...
```
  <figcaption>The engine's actual output, run in-process against the seeded demo experiment at an interim look. This run is the whole thesis in one screen: the naive p-value (0.0337) clears α=0.05 and looks shippable, but the O'Brien-Fleming boundary at 71.6% information demands |z| &gt; 2.316 and the observed z is 2.127 — so the honest verdict is keep running. A t-test endpoint would have called it a winner today.</figcaption>
</figure>
...
Two screens carry the argument: the operator view that says what needs attention next, and the results view where the statistical engine either clears a variant to ship or refuses to. Both are running against the seeded demo tenant.

<div class="case-gallery">
  <figure class="case-gallery__figure">
    <a class="case-gallery__link" href="/images/projects/experiment-hub/dashboard.png" target="_blank" rel="noopener noreferrer">
      <img src="/images/projects/experiment-hub/dashboard.png" alt="ExperimentHub dashboard showing running experiments, draft queue, assignments today, enabled flags, a recent-experiments list, and a lifecycle portfolio snapshot." loading="lazy" />
    </a>
    <figcaption>Command centre: portfolio counters plus the next action each experiment needs, so lifecycle drift is visible rather than discovered.</figcaption>
  </figure>
  <figure class="case-gallery__figure">
    <a class="case-gallery__link" href="/images/projects/experiment-hub/results.png" target="_blank" rel="noopener noreferrer">
      <img src="/images/projects/experiment-hub/results.png" alt="ExperimentHub results view for a concluded experiment showing p-value, power, effect size, a variant performance table with lift, and confidence-interval and conversion-over-time charts." loading="lazy" />
    </a>
    <figcaption>Results: p-value, power, and effect size sit above the variant table, so the read is a decision and not a vibe.</figcaption>
  </figure>
</div>
