...
subtitle: "Two-user PWA for a long-distance couple - cycle phase, mood, and stress become shared context driving daily guidance, with privacy enforced by the database, not the UI."
role: "Solo Builder · Product & Engineering"
duration: "Aug 2026 – present"
stack: ["React 19", "TypeScript", "Tailwind v4", "Supabase", "Dexie", "PWA"]
tldr: "Most cycle apps treat one partner as the user and the other as an outsider - or worse, treat her as the subject. Better-Half is built for two first-class users in a long-distance relationship: her phase, mood, and stress (shared per-category, on her terms) drive daily guidance for her, for him, and for the couple. Privacy is enforced with row-level security proven by 575 live policy checks, the app works offline-first on unreliable connections, and the whole thing runs on $0/month infrastructure."
metrics:
...
- **Offline is the normal case.** Hostel wifi and metro commutes mean hours offline daily. Every write queues locally (Dexie/IndexedDB) with per-field merge, dead-lettered failures, and idempotent replay - "saved" means saved.
...
- Private deployment, by design - this one ships to exactly two users
